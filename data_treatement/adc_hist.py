...
def run(hist, options, h_type='ADC', prev_fit_result=None):
    ...
    logger.info('Treating the batch #%d of %d events' % (batch_num, n_batch))
    for file in options.file_list:
        ...
        logger.info('--|> Moving to file %s' % _url)
        ...
        for event in inputfile_reader:
            ...
            if (n_evt - n_batch * batch_num) % n_batch / 100 == 0:
                logger.info("Progress {:2.1%}".format(float(n_evt - batch_num * n_batch) / n_batch), end="\r")
            for telid in event.r1.tels_with_data:
                if n_evt % n_batch == 0:
                    ...
                    batch = None
                    batch_num += 1
                    logger.info('Reading  the batch #%d of %d events' % (batch_num, n_batch))
                # Get the data
