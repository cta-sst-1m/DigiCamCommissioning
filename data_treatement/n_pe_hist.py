...
def run(hist, options, peak_positions=None, charge_extraction = 'amplitude', baseline=0.):
    ...
    for file in options.file_list:
        if level > len(options.shower_ids) - 1:
            break
        # Get the file
        ...
        for event in inputfile_reader:
            ...
            for telid in event.r0.tels_with_data:
                ...
                if evt_num % options.evt_per_shower == 0:
                    batch_index = 0
                    if charge_extraction == 'integration_sat' or charge_extraction=='integration':
                        #print(batch)
                        hist.data[level]=np.mean(batch,axis=-1)
                        # Reset the batch
                        batch = np.zeros((len(options.pixel_list), options.evt_per_shower),dtype=int)
                    if charge_extraction == 'baseline':
                        pass
                        #batch = np.zeros((len(options.pixel_list*(1+ options.n_bins - options.window_width)), options.evt_per_shower), dtype=int)
                    level = int(evt_num / options.evt_per_shower)
                    if level > len(options.shower_ids) - 1:
                        break
                    if options.verbose:

                        log.debug('--|> Moving to DAC Level %d' % (options.shower_ids[level]))
                if options.evt_per_shower<=1000:
                    pbar.update(1)
                else:
                    if evt_num % int(options.evt_per_shower/100)== 0:
                        pbar.update(int(options.evt_per_shower/100))
                ...
                if charge_extraction == 'global_max':
                    ...
                    batch[..., batch_index] = full_integration - baseline[level, :]
                    batch_index += 1
                    ...
                    #batch[:,evt_num%(options.evt_per_shower*integral.shape[1]):evt_num%(options.evt_per_shower*integral.shape[1])+integral.shape[1]]=np.apply_along_axis(integrate_trace,-1,data)
                    hist.fill(integral,indices=(level,))

                if batch_index % options.evt_per_shower == 0:
                    batch_index = 0
