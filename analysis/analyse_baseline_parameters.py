...
def create_histo(options):
    ...
    # Create an histogram of shape (len(options.pixel_list), options.evt_max, 2) to hold
    # per pixel and per event the baseline mean and rms as obtained from options.baseline_per_event_limit
    # events
    adcs = histogram.Histogram(bin_center_min=0, bin_center_max=2,
                               bin_width=1, data_shape=(len(options.pixel_list), options.evt_max,),
                               label='Mean and RMS of the baseline', xlabel='Mean RMS', ylabel='entries')
