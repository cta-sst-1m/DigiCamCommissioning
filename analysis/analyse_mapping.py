...
# internal modules
from data_treatement import mapping_hist
...
def create_histo(options):
    ...
    mapping = histogram.Histogram(bin_center_min=0, bin_center_max=len(options.evt_max)-1,
                               bin_width=1, data_shape=(len(options.pixel_list),),
                               label='Mapping',xlabel='Pixel',ylabel = '$\mathrm{N_{entries}}$')
    ...
    mapping_hist.run(mapping, options, peak_positions=peaks.data)

    # Save the histogram
    mapping.save(options.output_directory + options.histo_filename)
...
def display_results(options):
    ...
    # Load the histogram
    mapping = histogram.Histogram(filename=options.output_directory + options.histo_filename)
